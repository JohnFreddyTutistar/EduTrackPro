...
export class SharedService {
  ...
  postApplicantsCallHistory(
    id: number,
    data: ICallHistory
  ): Observable<ICallHistory[]> {
    return this.http.post<ICallHistory[]>(
      this.url + `/api/v1/history/${id}`,
      data
    );
  }
  ...
  getDataReviwersById(id: string): Observable<any> {
